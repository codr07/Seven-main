  ...
  container.innerHTML = `
    <section class="overflow-hidden rounded-2xl border border-black/15 bg-white/80 shadow-lg backdrop-blur-sm">
      <div class="relative h-56 w-full sm:h-72 lg:h-80">
        <img src="${note.coverImage || "public/assets/images/img/thumb.png"}" alt="${note.title}" class="h-full w-full object-cover" />
        ...
        <div class="absolute bottom-0 left-0 right-0 p-5 sm:p-7">
          <span class="mb-3 inline-flex items-center rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-gray-900">${note.category}</span>
          <h1 class="text-2xl font-bold text-white sm:text-3xl lg:text-4xl">${note.title}</h1>
          <p class="mt-2 max-w-3xl text-sm text-white/95 sm:text-base">${note.shortDescription || "Concise, exam-focused notes designed for faster revision."}</p>
        </div>
      </div>
      ...
      <div class="grid gap-6 p-5 sm:p-7 lg:grid-cols-3">
        ...
        <aside class="space-y-4">
          <div class="rounded-xl border border-black/10 bg-white p-5">
            ...
            <p class="mt-1 text-2xl font-bold text-gray-900">${note.price || "TBD"}</p>
          </div>
