  ...
  container.innerHTML = `
    <section class="overflow-hidden rounded-2xl border border-black/15 bg-white/80 shadow-lg backdrop-blur-sm">
      <div class="relative h-56 w-full sm:h-72 lg:h-80">
        <img src="${service.coverImage || "public/assets/images/img/thumb.png"}" alt="${service.title}" class="h-full w-full object-cover" />
      ...
      <div class="grid gap-6 p-5 sm:p-7 lg:grid-cols-3">
        ...
        <aside class="space-y-4">
          <div class="rounded-xl border border-black/10 bg-white p-5">
            ...
            <p class="mt-1 text-2xl font-bold text-gray-900">${service.price || "TBD"}</p>
          </div>

          <div class="rounded-xl border border-black/10 bg-white p-5">
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Service Code</p>
            <p class="mt-1 text-base font-bold text-gray-900">SV-${service.rank}</p>
          </div>

          <a href="${service.link || "#"}" target="_blank" rel="noopener noreferrer" class="inline-flex w-full items-center justify-center rounded-lg border border-black/20 bg-black px-4 py-3 text-sm font-semibold text-white hover:bg-white hover:text-black transition-colors">
            Buy Service
          </a>
        </aside>
      </div>
